...
import {IncomeEditAction, IncomeUpdateAction} from "../../redux/actions/IncomeAction";


const IncomeEdit = ({ match, history }) => {
  ...
  const incomeEdit = useSelector(state => state.incomeEdit)
  const { incomeedit } = incomeEdit

  useEffect(() => {
    dispatch(IncomeEditAction(incomeId))
  }, [dispatch, incomeId])
  // eslint-disable-next-line react-hooks/rules-of-hooks
  const formik = useFormik({
    initialValues: incomeedit ? incomeedit : {
      date: "",
      category: "",
      quantity: "",
      total_amount: ""
    ...
    onSubmit: (name) => {
      dispatch(IncomeUpdateAction({ name }, incomeId,() => {
